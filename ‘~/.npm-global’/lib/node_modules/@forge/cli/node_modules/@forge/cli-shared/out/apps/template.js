...
class ZipTemplateExtractor {
    ...
    async moveArchiveFilesToDirectory(extractDirectory, destDirectory) {
        ...
        const templateEntries = fs_1.default.readdirSync(templateRepoDirectory);
...
class TemplateFetchError extends Error {
    constructor(message) {
        super(ui_1.Text.error.templateFetchFailed(message));
    }
}
...
class TemplateServiceLister {
    ...
    async getAvailableTemplates() {
        ...
        if (!res.ok) {
            throw new TemplateFetchError(res.statusText);
        }
        ...
        if (this.isValidTemplatesResponse(json)) {
            ...
            throw new TemplateFetchError('Format error');
        }
    }
}
